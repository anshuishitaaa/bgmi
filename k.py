...
# Admin user IDs
admin_id = ["1086318600","6552242136","1695959688"]
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in user_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 120:
                response = "You Are On Cooldown 🙂. Please Wait 2min Before Running The /attack Command Again."
                bot.reply_to(message, response)
                return
        ...
        response = "🚫 Unauthorized Access!🚫Please buy access :-@AEGON_THE_CONQUEROR_bot"
...
def welcome_start(message):
    ...
    response = f'''🫡WELCOME TO  @AEGON_THE_CONQUEROR_bot🚀 '''
    bot.reply_to(message, response)
...
def welcome_rules(message):
    user_name = message.from_user.first_name
    response = f'''
1. ANY PROBLEM DM HERE -> @AEGON_THE_OWNER
    bot.reply_to(message, response)
